...
def _(PlanckDetectorsData, lmax):
    ...
    det_planck_data=PlanckDetectorsData(detector_set="100GHz", #you can refer to list_planck() to check the different set of detectors 
...
def _(SkyData, det_planck_data, lmax, nside):
    sky=SkyData(nside=nside,lmax=lmax,apply_pixel_window=False,temperature_only=False)
...
def _(compute_convolved_planck_map, det_planck_data, sky):
    TQU_conv,inverse_mm=compute_convolved_planck_map(sky_data=sky,
...
def _(det_planck_data, hp, nside, plt, sky):
    input_map=hp.alm2map(sky.alms_dict[det_planck_data.detector_names[0]],nside=nside,pol=True)
...
def _(lmax, np, plt, spect_conv, spect_input):
    ...
    for ax, idx, cur_label in zip(axes, spec_order, spec_labels):
        ...
        if cur_label in ["TB", "EB"]:
            ax.plot(ells, np.zeros_like(ells), "--", color="green", label="Input (0)")
        else:
            ax.plot(ells, spect_input[idx][:lmax+1], "--", color="green", label="Input")
...
@app.cell
def _(det_planck_data, hp, np, plt):
    ...
    def _():
        ...
        for s in np.arange(1, mmax+3, 1):
            ...
            # Get the data
            map_data = h_n_spin_dict[s][det_index].real#-h_n_spin_dict[s][det_index+1].real
            mean_val = np.mean(map_data)
        
            # Create the mollview projection using the axis
            hp.mollview(map_data, 
                        title=f"$\\tilde h _{{{s}}}$",
                        min=np.mean(map_data) - 3*np.std(map_data),
                        max=np.mean(map_data) + 3*np.std(map_data), # Consistent color scale
                        cmap='RdBu_r',        # Diverging colormap
                        unit=r'$\mu K$',
                        hold=True,
                        sub=(3, 2, subplot_idx + 1),
                        format="%.2e", 
                        )
        
            # Add mean value annotation
            ax.annotate(f'Mean: {mean_val:.1e}', 
        ...
        # Add a main title
        # fig.suptitle(f'Spin Maps for Detector: {det_names[det_index]}', 
...
def _(hp, inverse_mm, np, nside, plt):
    def _():
        ...
        extended_mapmaking_matrix = np.ones((nstokes,nstokes,)+total_mask.shape).squeeze() * hp.UNSEEN

        # value = 10
        plt.figure(figsize=(10,8))
        extended_mapmaking_matrix[:,:, total_mask != 0] = mapmaking_matrix.T.real
        for i in range(nstokes**2):
            row_ = i//nstokes
            col_ = i%nstokes
        

            hp.mollview(extended_mapmaking_matrix[row_,col_,:], sub=(nstokes,nstokes,i+1), cmap="seismic",
                        title=f'M element ({row_},{col_}) real \n <mean> = {mapmaking_matrix[:,row_,col_].real.mean():.2e}')#, min=-value, max=value)

        plt.figure(figsize=(10,8))
        extended_mapmaking_matrix[:,:, total_mask != 0] = mapmaking_matrix.T.imag
        for i in range(nstokes**2):
            row_ = i//nstokes
            col_ = i%nstokes
        return hp.mollview(extended_mapmaking_matrix[row_,col_,:], sub=(nstokes,nstokes,i+1),cmap="seismic",
                        title=f'M element ({row_},{col_}) imag \n <mean> = {mapmaking_matrix[:,row_,col_].imag.mean():.2e}')#, min=-value, max=value)
...
@app.cell
def _(GridSpec, hp, np, plt, spin_syst):
    def _():
        # Create figure with GridSpec for better control
        fig = plt.figure(figsize=(15, 10), dpi=200)
        gs = GridSpec(3, 3, figure=fig, hspace=0.3, wspace=0.1)
        mmax=4
        det_index=0
        # Determine color scale limits for consistent comparison
        all_data = []
        for s in np.arange(0, mmax+1, 2):
            all_data.append(spin_syst[s][det_index].real)
        ...
        for s in np.arange(0, mmax+1, 1):
            ...
            # Get the data
            map_data = spin_syst[s][det_index].real
            mean_val = np.mean(map_data)
